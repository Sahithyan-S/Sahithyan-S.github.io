/* ==========================================================================
   ...
   Moving the cursor near a vertex "fires" it: impulses travel along mesh
   edges to connected vertices, which fire in turn (with refractory
   periods) so cascades ripple across the brain and die out naturally.

   Adapted from the brain-mesh hero built for torwager/elements-of-fmri-tutorials
   (MIT licensed). Mesh/triangulation/firing logic is unchanged; the palette
   is retheme'd to this site's warm serif design and now tracks the site's
   light/dark toggle instead of a single fixed color scheme.
   ========================================================================== */
(function () {
  ...
  // --- palettes (light / dark) --------------------------------------------
  // Shape mirrors the original engine's color object exactly; only the
  // values change, and which one is active is re-evaluated on theme change.
  const LIGHT = {
    edge: 'rgba(139,69,19,0.16)',
    edgeHot: '196,140,72',
    vert: '#cfc0a4',
    vertEdge: '#b3a37f',
    hot: '#c97a2c',
  ...
  const DARK = {
    edge: 'rgba(212,163,115,0.20)',
    edgeHot: '240,190,120',
    vert: '#5d5346',
    vertEdge: '#7c6f5c',
    hot: '#f0b768',
    hotCore: '#fff6e0',
    membrane: '235,178,102',
  };
  ...
  let C = isDarkTheme() ? DARK : LIGHT;
  function refreshPalette() { C = isDarkTheme() ? DARK : LIGHT; }

  // Watch the manual toggle (data-theme attribute on <html>) and the OS-level
  // scheme, so the mesh recolors instantly without touching the toggle script.
  ...
  function build() {
    ...
    // (scaled a bit smaller than the reference implementation)
    const margin = 0.06 * Math.min(W, H);
    const availW = W > 900 ? W * 0.46 : W * 0.82;
    const availH = H * 0.66;
  ...
  function draw() {
    ...
    // membrane shimmer: faint fill on lit triangles
    for (const t of tris) {
    ...
    for (const n of nodes) {
      if (n.light > 0.02) {
        ...
        g.addColorStop(0, `rgba(${C.edgeHot},${(0.55 * n.light).toFixed(3)})`);
        g.addColorStop(1, `rgba(${C.edgeHot},0)`);
        ctx.fillStyle = g; ctx.beginPath(); ctx.arc(n.x, n.y, n.r + 20 * n.light, 0, Math.PI * 2); ctx.fill();
      }
      ctx.beginPath(); ctx.arc(n.x, n.y, n.r, 0, Math.PI * 2);
      ctx.fillStyle = n.light > 0.02 ? C.hot : C.vert;
      ctx.fill();
    ...
    for (const im of impulses) {
      ...
      for (let s = 6; s >= 1; s--) {
        ...
        ctx.fillStyle = `rgba(${C.edgeHot},${(0.5 - s * 0.07).toFixed(3)})`; ctx.fill();
      }
      const p = lerpPoint(im.e, T);
      const g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, 14);
      g.addColorStop(0, C.hotCore); g.addColorStop(0.35, `rgba(${C.edgeHot},0.6)`); g.addColorStop(1, `rgba(${C.edgeHot},0)`);
